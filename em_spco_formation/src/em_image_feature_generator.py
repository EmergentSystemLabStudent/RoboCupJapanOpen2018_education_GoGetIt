...
def main(argv):
    ...
    DATASET_FOLDER = "../training_data/trial"

    # Load numpy array (.npy), directory glob (*.jpg), or image file.
    all_file=glob.glob(DATASET_FOLDER+"/image/*."+args.ext)
    # print all_file
    convert = lambda text: int(text) if text.isdigit() else text 
    alphanum_key = lambda key: [ convert(c) for c in re.split('([0-9]+)', key) ] 
    all_file.sort(key=alphanum_key)
    os.mkdir(DATASET_FOLDER+"/feature_vector")
    os.mkdir(DATASET_FOLDER+"/feature_rank")
    ...
    for f in all_file:
        ...
        fw=open(DATASET_FOLDER+"/feature_vector/"+str(i)+".txt",'w')
        # print feat
        ...
        fw=open(DATASET_FOLDER+"/feature_rank/"+str(i)+".txt",'w')
