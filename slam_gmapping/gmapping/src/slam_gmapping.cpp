...
{
  pose_publisher_ = private_nh_.advertise<geometry_msgs::PoseStamped>("/best_pose", 1, true);
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  cout << "\t\tBEST_POSE" << best.pose.x << "," << best.pose.y << "," << best.pose.theta <<endl;
  geometry_msgs::PoseStamped BestPose;
  BestPose.pose.position.x = best.pose.x;
  BestPose.pose.position.y = best.pose.y;
  BestPose.pose.position.z = 0;
  BestPose.pose.orientation = tf::createQuaternionMsgFromYaw(best.pose.theta);

  pose_publisher_.publish(BestPose);
